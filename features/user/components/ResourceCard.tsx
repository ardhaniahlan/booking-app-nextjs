...
const ResourceCard = ({ 
  id, title, category, description, price, priceUnit, imageUrl, rating = 5.0, isActive 
}: ResourceCardProps) => {
  return (
    <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden hover:shadow-xl transition-all duration-300 flex flex-col group">
      ...
      <div className="relative h-56 w-full bg-slate-100 overflow-hidden">
        ...
        <div className="absolute top-4 left-4 bg-white/90 backdrop-blur-sm px-3 py-1.5 rounded-full flex items-center gap-2 shadow-sm">
          <div className={`w-2 h-2 rounded-full ${isActive ? 'bg-blue-600 animate-pulse' : 'bg-slate-400'}`}></div>
      ...
      <div className="p-5 flex flex-col flex-1">
        ...
        <h3 className="font-bold text-xl text-slate-900 mb-2 line-clamp-1">{title}</h3>
        ...
        <div className="flex justify-between items-end pt-4 border-t border-slate-100 mt-auto">
          <div>
            ...
            <div className="flex items-baseline gap-1">
              ...
              <span className="text-sm font-medium text-slate-500">{priceUnit}</span>
            </div>
          </div>
